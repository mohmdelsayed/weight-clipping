...
class ShrinkAndPerturbAdam(torch.optim.Optimizer):
    def __init__(self, params, sigma=0.01, optimizer=torch.optim.Adam, **kwargs):
        defaults = dict(sigma=sigma)
        super(ShrinkAndPerturbAdam, self).__init__(params, defaults)
...
class ShrinkAndPerturbSGD(torch.optim.Optimizer):
    def __init__(self, params, sigma=0.01, optimizer=torch.optim.SGD, **kwargs):
        defaults = dict(sigma=sigma)
        super(ShrinkAndPerturbSGD, self).__init__(params, defaults)
        self.optimizer = optimizer(self.param_groups, **kwargs)
        self.param_groups = self.optimizer.param_groups
        self.defaults.update(self.optimizer.defaults)
